...
#include <vector>
#include "ResonantLPFwMassberg.h"
...
*/
CResonantLPFwMassberg::CResonantLPFwMassberg()
{
	...
	m_PlugInName = "ResonantLPFwMassberg";
...
*/
CResonantLPFwMassberg::~CResonantLPFwMassberg(void)
{
...
*/
bool __stdcall CResonantLPFwMassberg::initialize()
{
	// Add your code here
...
processRackAFXMessage()
	...
	See the website www.willpirkle.com for more details
*/
void __stdcall CResonantLPFwMassberg::processRackAFXMessage(UINT uMessage, PROCESS_INFO& processInfo)
{
...
*/
bool __stdcall CResonantLPFwMassberg::prepareForPlay()
{
	// Add your code here:
...
*/
bool __stdcall CResonantLPFwMassberg::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
...
//
bool __stdcall CResonantLPFwMassberg::userInterfaceChange(int nControlIndex)
{
	// dont need switch since any parameter change will require new calculations
	// update coeffs for next time
	// use choice to change
	if(m_f_select)
		calculateMassbergLPFCoeffs(m_f_fc_Hz, m_f_Q);
	else
		calculateLPFCoeffs(m_f_fc_Hz, m_f_Q);
...
bool __stdcall CResonantLPFwMassberg::checkUpdateGUI(int nControlIndex, float fValue, CLinkedList<GUI_PARAMETER>& guiParameters, bool bLoadingPreset)
{
...
bool __stdcall CResonantLPFwMassberg::processAuxInputBus(audioProcessData* pAudioProcessData)
{
...
*/
bool __stdcall CResonantLPFwMassberg::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
/* processAudioBuffer
	...
	******************************
	...
	m_HostProcessInfo.uTimeSigDenomintor = Host Time Signature Denominator (if supported by host)
*/
bool __stdcall CResonantLPFwMassberg::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CResonantLPFwMassberg::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CResonantLPFwMassberg::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CResonantLPFwMassberg::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CResonantLPFwMassberg::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CResonantLPFwMassberg::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CResonantLPFwMassberg::midiClock()
{
...
bool __stdcall CResonantLPFwMassberg::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
*/
void CResonantLPFwMassberg::doVSTSampleAccurateParamUpdates()
{
...
void* __stdcall CResonantLPFwMassberg::showGUI(void* pInfo)
{
...
bool __stdcall CResonantLPFwMassberg::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! -------------------------------------- //
...
// **--0xDEA7--**
	...
	ui2->cControlName = "Selection (1 is Massberg)";
...
void CResonantLPFwMassberg::calculateLPFCoeffs(float fCutoffFreq, float fQ)
{
	// use same terms as in book:
	float theta_c = 2.0 * pi * fCutoffFreq / (float)m_nSampleRate;
	float d = 1.0 / fQ;

	// for calculating coeffs
	float fBeta = .5 * (1.0 - ((d / 2.0)*(sin(theta_c)))) / (1 + ((d / 2.0)*(sin(theta_c))));
	float fGamma = (.5 + fBeta)*(cos(theta_c));
	float fAlpha = (.5 + fBeta - fGamma) / 2.0;
	...
	// right channel (copy)
	m_RightLPF.m_f_a0 = m_LeftLPF.m_f_a0;
	m_RightLPF.m_f_a1 = m_LeftLPF.m_f_a1;
	m_RightLPF.m_f_a2 = m_LeftLPF.m_f_a2;
	m_RightLPF.m_f_b1 = m_LeftLPF.m_f_b1;
	m_RightLPF.m_f_b2 = m_LeftLPF.m_f_b2;
}

void CResonantLPFwMassberg::calculateMassbergLPFCoeffs(float fCutoffFreq, float fQ)
{
	// note: there are a few typos in the book as specified here:
	// http://www.willpirkle.com/wp-content/uploads/2013/02/MassbergCorrection.jpg
	// * denotes edit to design EQs in book
	...
	// *calculate g1
	float g1_den_1 = pow((2.0 - pow(((sqrt(2.0) * pi) / theta_c), 2.0)), 2.0);
	float g1_den_2 = pow(((2.0 * pi) / (fQ * theta_c)), 2.0);
	float g1 = 2.0 / sqrt(g1_den_1 + g1_den_2);
	
	// *calculate big_omega_s depending on Q:
	float big_omega_s;
	if (fQ > sqrt(.5))
	{
		// calculate gr
		float gr_num = 2.0 * pow(fQ, 2.0);
		float gr_den = sqrt((4.0 * pow(fQ, 2.0)) - 1);
		float gr = gr_num / gr_den;

		// calculate small_omega_r
		float sor_term = sqrt(1 - (1 / (2 * pow(fQ, 2.0))));
		float small_omega_r = theta_c * sor_term;

		// calculate big_omega_r
		float big_omega_r = tan(small_omega_r / 2.0);

		// calculate big_omega_s
		float bos_rad_num = pow(gr, 2.0) - pow(g1, 2.0);
		float bos_rad_den = pow(gr, 2.0) - 1.0;
		big_omega_s = big_omega_r * pow((bos_rad_num / bos_rad_den), .25);
	}
	else
	{
		// *calculate small_omega_m
		float som_rad_num_2 = 1.0 / (pow(fQ, 2.0));
		float som_rad_num_3a = (1.0 - (4.0 * pow(fQ, 2.0))) / pow(fQ, 4.0);
		float som_rad_num_3b = 4.0 / g1;
		float som_rad_num_3 = sqrt(som_rad_num_3a + som_rad_num_3b);
		float small_omega_m = theta_c * sqrt((2.0 - som_rad_num_2 + som_rad_num_3) / 2.0);

		// calculate big_omega_m
		float big_omega_m = tan(small_omega_m / 2.0);

		// calculate big_omega_s
		big_omega_s = (theta_c * pow((1 - pow(g1, 2.0)), .25)) / 2.0;
		big_omega_s = min(big_omega_s, big_omega_m);
	}

	// calculate the pole and zero frequencies (small_omega),
	float small_omega_p = 2.0 * atan(big_omega_s);
	float small_omega_z = 2.0 * atan2(big_omega_s, sqrt(g1));

	// gains (g),
	// *gp
	float gp_den_1 = pow((1.0 - pow((small_omega_p / theta_c), 2.0)), 2.0);
	float gp_den_2 = pow((small_omega_p / (fQ * theta_c)), 2.0);
	float gp = 1.0 / sqrt(gp_den_1 + gp_den_2);
	
	// *gz
	float gz_den_1 = pow((1.0 - pow((small_omega_z / theta_c), 2.0)), 2.0);
	float gz_den_2 = pow((small_omega_z / (fQ * theta_c)), 2.0);
	float gz = 1.0 / sqrt(gz_den_1 + gz_den_2);

	// and Qs
	// Qp
	float Qp_num = g1 * (pow(gp, 2.0) - pow(gz, 2.0));
	float Qp_den = (g1 + pow(gz, 2.0)) * pow((g1 - 1.0), 2.0);
	float Qp = sqrt(Qp_num / Qp_den);

	// Qz
	float Qz_num = pow(g1, 2.0) * (pow(gp, 2.0) - pow(gz, 2.0));
	float Qz_den = pow(gz, 2.0) * (g1 + pow(gp, 2.0)) * pow((g1 - 1.0), 2.0);
	float Qz = sqrt(Qz_num / Qz_den);

	// for calculating coeffs
	float fGamma = pow(big_omega_s, 2.0) + ((1.0 / Qp) * big_omega_s) + 1.0;
	float fAlpha0 = pow(big_omega_s, 2.0) + ((sqrt(g1) / Qz) * big_omega_s) + g1;
	float fAlpha1 = 2.0 * (pow(big_omega_s, 2.0) - g1);
	float fAlpha2 = pow(big_omega_s, 2.0) - ((sqrt(g1) / Qz) * big_omega_s) + g1;
	float fBeta1 = 2.0 * (pow(big_omega_s, 2.0) - 1.0);
	float fBeta2 = pow(big_omega_s, 2.0) - ((1.0 / Qp) * big_omega_s) + 1.0;

	// left channel
	m_LeftLPF.m_f_a0 = fAlpha0 / fGamma;
	m_LeftLPF.m_f_a1 = fAlpha1 / fGamma;
	m_LeftLPF.m_f_a2 = fAlpha2 / fGamma;
	m_LeftLPF.m_f_b1 = fBeta1 / fGamma;
	m_LeftLPF.m_f_b2 = fBeta2 / fGamma;
